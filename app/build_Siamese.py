...
Contrastive_m_list = [10, 20]
eval_threshold_list = [5, 10]
batch = 128
...
for m, th in zip(Contrastive_m_list, eval_threshold_list):
    ...
    cross_validation(model_params = model_params, dataset = CV_dataset, k = 5, loss_fn = loss, eval_threshold = eval_threshold, device = device, out_dir = out_dir, batch_size = batch, nepochs = 10, save = True, random_state = 1)
